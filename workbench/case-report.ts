...
/**
 * СВЁРТКА ПОВТОРОВ «×N» (JIG-30). Матрица печатает по строке на УЗЕЛ внутри
 * ячейки (объявленная цель, объявленное узкое поле), и компонент вроде
 * `EventCalendar/week` кладёт в такую ячейку десятки одинаковых слотов —
 * тысячи побайтово одинаковых строк подряд, за которыми тонет вердикт.
 * Одинаковые строки сворачиваются в ОДНУ, на месте ПЕРВОГО появления, с
 * суффиксом «  ×N» (два пробела, знак ×, число) при N > 1; уникальные строки и
 * порядок первого появления не меняются. Повтор считается по строке ЦЕЛИКОМ,
 * а не по соседству: разбросанный повтор (не подряд) сворачивается так же, как
 * подряд идущий — предмет ЧИСТАЯ функция, печати не касается.
 */
