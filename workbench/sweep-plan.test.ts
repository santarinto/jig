...
const cell = (o: Partial<Cell>): Cell => ({
  c: 'X', caseId: 'base', theme: 'light', scale: 1, width: 360, target: 't', found: true, overflow: false, h: 30, w: 80, hit: true, ...o,
})

const spec = (o: Partial<SweepSpec> = {}): SweepSpec => ({
  c: 'X', cases: ['base'], widths: [360], targets: [{ name: 't', selector: '.x' }], ...o,
})
...
describe('evaluate', () => {
  ...
  it('min называет ячейку и число', () => {
    ...
    expect(v).toEqual([{ rule: 'h >= 24', at: 'X/base light ×0.875 @360 t', got: '21.4', miss: 24 - 21.4 }])
  })

  it('grows по шкале сравнивает соседей ПРИ ПРОЧИХ РАВНЫХ, а не всех со всеми', () => {
    const cells = [
      cell({ scale: 1, width: 360, h: 30 }), cell({ scale: 1.5, width: 360, h: 45 }),
  ...
  it('same по теме — разброс выше допуска, ось в адресе вынесена', () => {
    ...
    expect(evaluate(cells, [{ metric: 'h', same: 'theme', tol: 0.5 }])).toEqual([
      { rule: 'h одинаково по theme ±0.5', at: 'X/base {theme} ×1 @360 t', got: 'разброс 1', miss: 1 },
    ])
  ...
  it('gt: после больше до', () => {
    ...
    expect(v.map((x) => x.at)).toEqual(['X/base light ×1 @360 t'])
  })
  ...
  it('fits и hit', () => {
    ...
    expect(v.map((x) => `${x.rule} ${x.at}`)).toEqual(['fits X/base light ×1 @360', 'hit X/base light ×1 @900 t'])
  })
  ...
  it('fits — одно нарушение на документ, сколько бы целей в нём ни мерилось', () => {
    ...
    expect(v.map((x) => x.at)).toEqual(['X/base light ×1 @360'])
  })
})
