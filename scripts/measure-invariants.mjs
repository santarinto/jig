...
// Хост 800 × 600, а не впритык: первая панель `flex: 0 1 auto` (DS-326)
  ...
  },
  {
    ...
    measure: () => {
      const read = (host) => {
        const label = host.querySelector('.ds-formrow__label')
        const control = host.querySelector('.ds-formrow__control')
        const formrow = host.querySelector('.ds-formrow')
        const lr = label.getBoundingClientRect()
        const cr = control.getBoundingClientRect()
        ...
        return {
          ...
          // Фактическая ширина строки (JIG-37) — по ней ниже закреплён порог
          // 24em как ЧИСЛО, а не кадр хоста: `.ds-card__body` забирает padding.
          formrowWidth: +fr.width.toFixed(2),
        }
      }
    ...
    expect: (m) => {
      ...
      // JIG-37: закрепляем САМО число 24em по фактической ширине строки
      // `.ds-formrow` на шкале 1.5 (24em × 14 × 1.5 = 504 px), а не только
      // неравенство поле/подпись выше — оно держится при любом пороге ≥
      // ~20.86em, и 22em (или 21em) прошёл бы зелёным без этой проверки.
      const THRESHOLD_PX = 504
      const TOL = 0.5
      const scale15 = m.filter((r) => r.sc === '1.5')
      const below = scale15.filter((r) => r.formrowWidth < THRESHOLD_PX - TOL)
      const above = scale15.filter((r) => r.formrowWidth > THRESHOLD_PX + TOL)
      if (below.length === 0 || above.length === 0) {
        return `развёртка на шкале 1.5 не покрывает обе стороны порога ${THRESHOLD_PX} px (24em × 14 × 1.5): ниже — ${below.length}, выше — ${above.length} из ${scale15.length} ячеек — случай не проверяет сам порог (санитар)`
      }
      const mismatched = scale15.filter((r) => {
        if (r.formrowWidth < THRESHOLD_PX - TOL) return r.side !== false
        if (r.formrowWidth > THRESHOLD_PX + TOL) return r.side !== true
        return false
      })
      if (mismatched.length) {
        return `порог ${THRESHOLD_PX} px (24em × 14 × 1.5) разошёлся с фактической раскладкой на ${mismatched.length} из ${scale15.length} ячеек шкалы 1.5: `
          + mismatched.slice(0, 6).map((r) => `w=${r.w} — строка ${r.formrowWidth} px, раскладка «${r.side ? 'сбоку' : 'сверху'}», ожидалось «${r.formrowWidth > THRESHOLD_PX ? 'сбоку' : 'сверху'}»`).join('; ')
          + (mismatched.length > 6 ? `; и ещё ${mismatched.length - 6}` : '')
      }
      return true
    },
  },
  {
    ...
    why: 'JIG-38, найдено на верстаке 24.09.2026: кадр 528, шкала 1.5, поле'
      ...
      + ' Мутация (ревью JIG-38): снять `+ var(--ds-space-2)` из резерва в'
      + ' `DatePicker.css` — граница контента подходит вплотную к крестику'
      + ' (зазор ~0), нахлёста при этом всё ещё нет, и случай красит вторым'
      + ' утверждением («зазор текст—крестик у́же токена --ds-space-2»);'
      + ' откат обратной правкой.',
    html: (() => {
      const SCALES = ['0.875', '1', '1.15', '1.5']
      const SIZES = ['md', 'sm']
      const cell = (sc, size) => `
    ...
    expect: (m) => {
      ...
      // JIG-38: одного отсутствия нахлёста мало — снятие `+ var(--ds-space-2)`
      // из резерва (`DatePicker.css`) подводит границу контента вплотную к
      // крестику (зазор ~0), а проверка выше это пропускает (нахлёста нет).
      // Зазор обязан быть не у́же самого токена `--ds-space-2` (4px × scale).
      const SPACE_2 = 4
      const TOL = 0.5
      const short = m.filter((r) => r.gap < SPACE_2 * parseFloat(r.sc) - TOL)
      if (short.length) {
        return `зазор текст—крестик у́же токена --ds-space-2 на ${short.length} из ${m.length}: `
          + short.map((r) => `size=${r.size}/шкала=${r.sc} — зазор ${r.gap}px < ${(SPACE_2 * parseFloat(r.sc)).toFixed(2)}px`).join('; ')
      }
      return true
    },
  },
