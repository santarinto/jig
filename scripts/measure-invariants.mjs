 ...
 * в кадре 360 — «телефон», самый узкий пресет верстака (`workbench/frame-width.ts`).
 *
 ...
 *    DataTable), 300 лежит с той же стороны, что и 360, и не спрашивает
 *    ничего нового;
 ...
 *
 * Счёт на 16.09.2026, посчитанный, а не прикинутый: ширину 360 или 900 берут
 * КОНТЕЙНЕРОМ 12 случаев (общее число случаев прогона здесь не называется
 * намеренно — его двигает каждая вторая задача, DS-230). Девять
 * спрашивают «держится ли на узком» — у
 * Pagination точка 300 была своя и раньше (полоса это кадр минус карточка),
 * пять получили её здесь, три — вместе с починкой своих
 * дефектов (DS-295, 296, 297). Оставшиеся три — DataTable,
 * где ширины лишь разведены вокруг порога скрытия колонок.
 */
const HOST_360 = hostOf(360)
 ...
 * больший прогон. Компактный вид, наоборот, обходит ВСЕ ПЯТЬ: его утверждение
 * («ряд номеров на кадре 360 не переносится») — про порог, а порог задаёт
 * ХУДШАЯ шкала, и худшая здесь 0.875, то есть та, что до DS-149 в списке
 * стояла первой лишь по случайности.
 */
const PAGER_SCALES = ['0.875', '1', '1.15', '1.25', '1.5']
const PAGER_LEGACY_SCALES = ['0.875', '1', '1.5']
/**
 * Ширины прежней группы — ПОЛОСЫ, а не кадра. 300 и 440 остались с
 * DS-142/149 (кадры 360 и 500 с карточкой), а 500 и 708 уехали на
 * DS-282 вместе с порогом складывания: на 708 полоса теперь сложена на
 * двух шкалах из трёх, и утверждение «ряд отдали там, где он ещё помещается»
 * стало неверным — ряду там не хватает КОЛОНКИ, что бы ни показывала ширина
 * полосы. 900 держит единицу порога (на 0.875 это 72.33em, на 1.5 — 41.71em —
 * было 77.9em / 44.9em при базе 13, DS-375 подняла её до 14 и оба числа
 * пересчитаны замером, но остались по разные стороны 59em, а в пикселях
 * содержимое отличается всего на 10px, и порог, переписанный в px, дал бы
 * один ответ на обеих шкалах). 1700 — ширина, на которой ПСЕВДОКЛЮЧЕВОЙ ряд
 * помещается в свою колонку на всех трёх шкалах (требование на шкале 1 —
 * 83.5em, было 84.8em при базе 13; ширина 1700 сама не пересчитывалась — она
 * держит фиксаж «есть у ряда своя колонка», а не сам порог, и осталась
 * зелёной на прогоне `npm run measure`): только там имеет смысл спрашивать
 * про центр диапазона.
 */
const PAGER_LEGACY_WIDTHS = [300, 440, 900, 1700]
 ...
 * стороны: то же соотношение, что у пары 300/360 в группе DS-142.
 *
 ...
 *
 * Шесть — не «побольше»: на кадре 360 они обязаны НЕ ВЛЕЗТЬ (иначе кейс мерит
 * бар, у которого переполнения нет вовсе), а на 768 обязаны влезть вместе со
 * слотом (иначе «места вдоволь» не отличить от «места нет»). Оба числа
 * проверяются самими утверждениями, а не подразумеваются.
 *
...
/**
 * Замер бара: документ, лента, слот. Общий для обоих кадров — 360 и 768
 * отвечают на разные вопросы ОДНОЙ И ТОЙ ЖЕ разметкой, и второй замер развёл
 * бы их незаметно.
 */
 ...
 * (DS-284).
 *
 * 360…440 — тот диапазон, где приёмка DS-236 увидела дефект глазами:
 * на 360 документ уезжал вбок на 57px, на 440 уже нет. Кадр здесь НАСТОЯЩИЙ
 * вьюпорт (`viewport`), а не ширина `body`: предмет — `scrollWidth` документа,
 * и он смотрит на вьюпорт.
 */
const TABS_ARROW_FRAMES = [360, 380, 400, 440]
 ...
 * верстака (`workbench/frame.css`), и замер владельца на «кадре 360» снят с
 * контейнера 300 — лента 194.7 плюс панель 105.3. Тот же приём, что у группы
 * Pagination, где ширина полосы это кадр минус карточка.
 */
const TABS_SIDE_FRAMES = [360, 440, 900]
 ...
 * утверждениями, а не подразумеваются: шапка на 360 при шкале 1.5 сворачивает
 * ВСЕ шесть действий, и точка с такой шириной молча мерила бы пустоту.
 */
const MORE_POINTS = ['light', 'dark'].flatMap((theme) => [
  { theme, sc: '1', c: 'CommandBar', id: 'many', w: 360 },
  { theme, sc: '1', c: 'AppBar', id: 'crowded', w: 520 },
  { theme, sc: '1.5', c: 'CommandBar', id: 'many', w: 360 },
...
const ANCHOR_POINTS = ['1', '1.5'].flatMap((sc) => [
  { sc, c: 'DropdownMenu', id: 'own-trigger', w: 300, align: 'start' },
  { sc, c: 'DropdownMenu', id: 'bare-trigger', w: 300, align: 'start' },
  { sc, c: 'CommandBar', id: 'many', w: 360, align: 'end' },
  { sc, c: 'AppBar', id: 'crowded', w: sc === '1' ? 520 : 360, align: 'end' },
])
...
const CASES = [
  ...
  },
  {
    ...
    why: 'auto-fit оставлял дорожку цвета сетки рядом с ней — 159px из 320',
    width: 320,
...
// Хост 800 × 600, а не впритык: первая панель `flex: 0 1 auto` (DS-326)
  ...
  },
  {
    ...
    why: 'без ограничения длинный заголовок растянул бы шапку на всю карточку. Проверялось регэкспом по `Form.css` — то есть НАЛИЧИЕМ строки `line-clamp: 2` в файле, а не её действием (DS-118). Три состояния меряются вместе и обязаны быть РАЗЛИЧИМЫ: без `--wrap` одна строка, с `--wrap` две, и текст на пять строк те же две',
    width: 320,
  ...
  },
  {
    ...
    width: 900,
    html: [1, 1.25, 1.5].flatMap((sc) => [HOST_360, 360, 768, 900].map((w) => `
  ...
  },
  {
    ...
    why: 'DS-169. У `.ds-table` есть осмысленный `min-width` — пол ширины колонки без `width` (DS-137), колонки уже него нечитаемы, — но своей прокрутки не было, и распор уходил наружу и двигал ДОКУМЕНТ: кадр 360 при шкале 1.5, таблица торчала до 395. Уехавшая вбок страница ломает не таблицу, а всё вокруг неё: шапку, боковую навигацию, всякий `position: fixed`. Задача три месяца была невыполнима — скроллер на обёртке резал меню строки, пока всплывающее лежало `absolute` внутри триггера (DS-240 это сняла). Дефект живёт на ПРОИЗВЕДЕНИИ узкого кадра и шкалы: на 360 × 1 его нет',
    ...
    // прибитые пиксели переполняли бы одинаково на всех трёх шкалах, и случай
    ...
    // обёртку» на всех трёх шкалах: на кадре 360 контейнерные запросы обнуляют
    // `sm` и `md` вместе с колонками, которые прячет `hideBelow`
    // (DS-138), — то есть пол честно ужимается до базовой группы.
    // Санитар это и назвал. Переполнять обязано то, что на узком кадре
    // остаётся.
    viewport: 360,
    html: [1, 1.25, 1.5].map((sc) => `
      <div class="ds-scale" style="--ds-ui-scale: ${sc}">
  ...
  },
  {
    ...
    width: 420,
    html: `<div class="ds-root" style="padding:8px;width:420px"><div class="ds-transcript">
  ...
  },
  {
    ...
    html: ['long', 'twins'].flatMap((set) => [HOST_360, 360, 900, 1189].flatMap((w) => ['1', '1.5'].map((sc) =>
  ...
  },
  {
    ...
    // Точка владельца (`HOST_360`) вернулась на DS-296. Она краснела не
    ...
    html: ['bar', 'line', 'weeks'].flatMap((kind) => [HOST_360, 360, 900, 1189].flatMap((w) => ['1', '1.5'].map((sc) =>
    ...
    expect: (m) => {
      ...
      // Ряды оси X LineChart: повёрнутые где угодно и прямые на 360 и шире.
      ...
      for (const r of m) {
        ...
        if (r.kind !== 'bar') {
          ...
          if (!turned && r.w >= 360) lineFlatWide++
        }
      ...
      if (!bad.length && lineFlatWide === 0) bad.push('на 360 и шире ни одного ПРЯМОГО ряда оси X LineChart — вся ось ушла в поворот')
      return bad.length === 0 || bad.join('; ')
    },
  },
  {
    ...
    // Точка владельца (`HOST_360`) вернулась на DS-295. Она краснела не
    ...
    // получают ровно 10.
    html: ['long', 'big'].flatMap((set) => [HOST_360, 360, 900].flatMap((w) => ['1', '1.5'].map((sc) =>
  ...
  },
  {
    ...
    html: ['line', 'weeks'].flatMap((kind) => [HOST_360, 360, 900].flatMap((w) => ['1', '1.5'].map((sc) =>
  ...
  },
  {
    ...
    bundle: 'scripts/measure-charts.tsx',
    html: ['line', 'bar'].flatMap((kind) => ['big', 'short'].flatMap((set) => [HOST_360, 360, 1189].flatMap((w) => ['1', '1.5'].map((sc) =>
  ...
  },
  {
    ...
    html: ['light', 'dark'].map((th) =>
      ...
      // Точка владельца (`HOST_360`) вернулась на DS-297. Она краснела не
      ...
      + ['base', 'weeks', 'big', 'eight'].flatMap((set) => [HOST_360, 360, 900].flatMap((w) => ['1', '1.5'].map((sc) =>
  ...
  // vw, position:fixed и на @media у потребителя.
  {
    ...
    why: 'на этом стоит весь верстак. Ширина, отданная div-у, показывает правду про @container и враньё про всё, что зависит от вьюпорта: в общем документе vw мерит ОКНО раннера (900px), и min(22.5rem, 90vw) выбирает rem-ветку — 360px; в кадре 90vw считается от вьюпорта кадра (320px) и даёт 288px — расхождение ровно в том, от чего считается vw',
    html: `
      <div id="host" style="width:320px"><div class="ds-drawer ds-drawer--left" id="d">панель</div></div>
    ...
    measure: async () => {
      ...
      fd.body.insertAdjacentHTML('beforeend', '<div class="ds-drawer ds-drawer--left" id="d">панель</div>')
      const w = (root) => +root.querySelector('#d').getBoundingClientRect().width.toFixed(1)
      return {
        doc: w(document),
        frame: w(fd),
        docViewport: document.documentElement.clientWidth,
        frameViewport: fd.documentElement.clientWidth,
        // Сосед с заведомо известным значением: если лист в кадр не доехал,
        // элемент не позиционирован, и все числа выше — про голый div.
        positioned: getComputedStyle(fd.querySelector('#d')).position,
      }
    },
    expect: (m) => {
      if (m.positioned !== 'fixed') return `лист стилей не доехал в кадр: position ${m.positioned}`
      if (m.frameViewport !== 320) return `вьюпорт кадра ${m.frameViewport}, а не 320 (полоса прокрутки?)`
      if (m.docViewport < 500) return `окно ${m.docViewport} — слишком узкое, пара не про что`
      const want = +(m.frameViewport * 0.9).toFixed(1)
      if (Math.abs(m.frame - want) > 1) return `в кадре ${m.frame}, а 90vw от ${m.frameViewport} это ${want}`
      if (Math.abs(m.doc - 360) > 1) return `в общем документе ${m.doc}, ожидалось 360 (min(22.5rem, 90vw от окна))`
      return true
    },
  },
  {
  ...
  },
  {
    ...
    expect: (m) => {
      ...
      if (m.viewport !== 360) return `вьюпорт кадра ${m.viewport}, а не 360 — мерили не кадр (полоса прокрутки?)`
  ...
  },
  {
    ...
    html: `${[420, 1600].map((w) => `<div style="width:${w}px"><dl class="ds-kv ds-kv--cols-1" id="kv${w}">
    ...
    measure: () => {
      ...
      return { narrow: row('kv420'), wide: row('kv1600') }
    },
    expect: (m) => {
      // На узком контейнере колонка обязана взять его целиком: иначе «ограничено»
      // означало бы «сломано на любой ширине».
      if (m.narrow.width < 400) return `на 420px колонка сжалась до ${m.narrow.width}px — предел режет там, где резать нечего`
  ...
  },
  {
    ...
    html: ['narrow', 'wide'].map((id) => `
      <div class="ds-table-wrap" id="${id}" style="width:${id === 'narrow' ? 342 : 1000}px;
           --ds-table-min-base: calc(192px + 80px + 64px + var(--ds-w-col-min));
           --ds-table-min-sm: 0px; --ds-table-min-md: 0px">
        <table class="ds-table ds-table--fixed" data-ds-managed-rows>
          <colgroup><col style="width:192px"><col style="width:80px"><col style="width:64px"><col></colgroup>
    ...
    expect: (m) => {
      ...
      if (a !== 192 || b !== 80 || c !== 64) {
        return `на просторе заданные ширины уехали: ${a}/${b}/${c} вместо 192/80/64 — пол начал раздавать место всем`
      }
      const rest = m.wide.cols[3]
      return rest > 600 || `на просторе колонка без ширины взяла ${rest}px вместо остатка (~664px)`
    },
  },
  {
    ...
    html: ['narrow', 'wide'].map((id) => `
      <div class="ds-table-wrap" id="hb-${id}" style="width:${id === 'narrow' ? 400 : 900}px;
    ...
    expect: (m) => {
      if (m.narrow.cells.length !== 3) {
        return `на 400px видно ${m.narrow.cells.length} колонки из 3 — hideBelow не сработал вовсе`
      }
  ...
  },
  {
    ...
    // Ширины подобраны так, чтобы пол упирался и ПОСЛЕ скрытия: 192 + 120 +
    // метрика = 376 против обёртки 342. Тогда одна величина — ширина таблицы —
    // различает ТРИ состояния, а не два:
    //   456 — пол держит скрытую колонку (дефект),
    //   376 — пол уменьшился на неё и остался полом (чинили это),
    //   342 — пола нет вовсе (правка, которая «починила» тесноту, убрав пол;
    //         колонка-остаток тогда снова уезжает — здесь в 30px).
    // На прежних числах (192+80+64) третье состояние было неотличимо от
    // второго: пол при 342 не упирался и без скрытой колонки.
    html: ['narrow', 'wide'].map((id) => `
      <div class="ds-table-wrap" id="fl-${id}" style="width:${id === 'narrow' ? 342 : 900}px;
           --ds-table-min-base: calc(192px + 120px + var(--ds-w-col-min));
           --ds-table-min-sm: 80px; --ds-table-min-md: 0px">
        <table class="ds-table ds-table--fixed" data-ds-managed-rows>
          <colgroup>
            <col style="width:192px">
            <col class="ds-table__hide-sm" style="width:80px">
    ...
    expect: (m) => {
      ...
      if (m.narrow.cols.length !== 3) {
        return `на 342px видно ${m.narrow.cols.length} колонки из 3 — hideBelow не сработал`
      }
      // Ожидаемое считается из тех же чисел, что и во фрагменте: 192 + 120 плюс
      // метрика на колонку без ширины. Литерал 376 здесь молча разошёлся бы с
      // фрагментом при первой же правке ширин.
      const want = 192 + 120 + m.floor
      ...
      const [a, b, c] = m.wide.cols
      return (a === 192 && b === 80 && c === 120)
        || `на просторе заданные ширины уехали: ${a}/${b}/${c} вместо 192/80/120`
    },
  },
  {
  ...
  },
  {
     ...
     *
     * Ширины 300 и 1700 — две ветки правила складывания одним прогоном: первая
     * складывается на всех шкалах (это полоса на кадре 360), вторая не
     * складывается ни на одной. 1700 стоит на месте прежней 708: с
    ...
    html: (() => {
      ...
      // Компактная группа: ВСЕ пять шкал, ОБА набора текста, кадр 360 с двух
      // сторон — полоса 300 (карточка со своим паддингом) и полоса 360
      // (карточка без него). Вторая ширина здесь не для полноты: именно она
      // задаёт порог. Полоса 300 требует от порога всего 23.35em, полоса 360 —
      // 28.24em (было 25.14em / 30.42em при базе `--ds-fs-base` 13, DS-375
      // подняла её до 14), и порог, снятый только с первой, на настоящем
      // телефоне с карточкой без паддинга ряд номеров опять бы перенёс.
      for (const sc of PAGER_SCALES) {
        for (const [set, t] of [['ru', RU], ['pseudo', PSEUDO]]) {
          for (const w of [300, 360]) out.push(bar(`cm-${scaleKey(sc)}-${set}-${w}`, sc, t, w))
        }
      }
    ...
    expect: (m) => {
      ...
      const narrow = ['pg-0_875-300', 'pg-1-300', 'pg-1_5-300']
      ...
      if (flat.length) return `на 300px полоса не сложилась: ${flat.join(', ')} — зоны сжаты в столбик вместо рядов`
      ...
      // --- DS-149: на кадре 360 полоса приходит к компактному виду ---
      ...
      const cm = keys('cm-')
      if (cm.length !== 20) {
        return `компактную группу обошли на ${cm.length} случаях вместо 20 (5 шкал × 2 набора текста × 2 ширины кадра 360)`
      }
      const busy = cm.filter((k) => m[k].navRows !== 1)
      if (busy.length) {
        return `на кадре 360 ряд номеров всё ещё переносится: ${busy.map((k) => `${k} в ${m[k].navRows} ряда при ${m[k].em}em`).join(', ')}`
      }
      const tall = cm.filter((k) => m[k].barRows !== 3)
      if (tall.length) {
        return `на кадре 360 полоса не уложилась в три ряда: ${tall.map((k) => `${k} в ${m[k].barRows}`).join(', ')}`
      }
  ...
  },
  {
    ...
    // меряется В ПОЛЁТЕ — 324 шириной, но с правым краем 684 при кадре 360.
    reducedMotion: 'reduce',
    ...
    expect: (m) => {
      ...
      if (edge > 400) return `кадр ${edge}px — это не узкий экран, поле viewport до страницы не доехало`
      const bad = []
      ...
      // 90vw от 360 — это 324 на всех трёх шкалах, потому что на каждой из них
      // побеждает доля экрана, а не rem.
      for (const sc of OVERLAY_SCALES) {
        const w = m[sc].drawer.w
        if (Math.abs(w - 324) > 0.5) bad.push(`×${sc}: Drawer ${w} вместо 324 (90vw от 360) — сдвинулся образец, по которому выправлено остальное`)
      }
      return bad.length === 0 || bad.join('; ')
    },
  },
  ...
  },
  {
    ...
    expect: (m) => {
      ...
      for (const sc of OVERLAY_SCALES) {
        ...
        if (v.tipLong.r > 360.5) bad.push(`×${sc}: длинная подсказка за правым краем — правый край ${v.tipLong.r}`)
      }
      return bad.length === 0 || bad.join('; ')
    },
  },
  ...
  },
  {
    ...
    html: `<div class="ds-scale" style="--ds-ui-scale:1;width:300px" id="sp-narrow"></div>
      <div class="ds-scale" style="--ds-ui-scale:1;width:300px" id="sp-far"></div>
    ...
    expect: (m) => {
      if (!(m.max > 40)) return `лента 300 не листается (запас ${m.max}) — замер не о прокрутке`
  ...
  },
  {
    ...
    expect: (m) => {
      ...
      // Пол 12em — ТОЛЬКО центру с полем ввода. Короткий чип `crowded` на
      // 300×1 влезает в строку своим min-content (бренд 70.1 + чип 89.8 + «Ещё»
      // 42.6 с зазорами = 242.5 при 268, замер 23.09.2026 с брендом «Курьер 7»), и второй ряд там означал бы, что
      // объявленный пол достался центру без поля. На 300×1.5 чип сам 125.6, и
      // та же сумма 337.9 при 252 — там второй ряд законен.
      const chip = m.find((r) => r.id === 'crowded' && r.w === 300 && r.sc === 1)
      if (chip && chip.stacked) bad.push('crowded 300px×1: короткий чип ушёл во второй ряд — пол 12em достался центру без поля ввода')
      const field = m.find((r) => r.id === 'search' && r.w === 300 && r.sc === 1)
      if (field && !field.stacked) bad.push('search 300px×1: поиск остался в строке — у поля ввода нет объявленного пола')
  ...
  },
  {
    ...
    html: [HOST_360, 360, 380, 400, 440, 520].flatMap((w) => ['1', '1.5'].map((sc) =>
    ...
    expect: (m) => {
      ...
      if (m.length !== 12) return `хостов ${m.length} вместо 12 — шесть ширин на две шкалы`
      const bad = []
      let tight = 0, roomy = 0
      for (const r of m) {
  ...
  ...TABS_SIDE_FRAMES.map((frame) => ({
    name: `Tabs position=left на кадре ${frame}: ниже порога лента приходит к верхней, выше — телу хватает колонки`,
    why: 'DS-285, найдено приёмкой DS-236 глазами. Пол из 236 действует только на `top`/`bottom` — у ленты в `--left` `min-width: 0`, — и на кадре 360 (контейнер 300, шкала 1.5) вертикальная лента забирала 194.7, телу оставалось 105.3 при слове «Содержимое» шириной 144: слово срезано правым краем, документ переполнен на 9px. Боковой бар на телефоне отдаёт вкладкам больше половины ширины — это не форма для телефона вовсе, поэтому ниже порога `position="left"`/`right` приходит к `top`. Решает ШИРИНА, как у автопорога Pagination (DS-149) и как `hideBelow` у DataTable: потребитель видит не то, что просил, но видит работающее, а ширину контейнера в момент отрисовки он всё равно не знает. Утверждение здесь про РАСКЛАДКУ и про обещание порога — телу не меньше `--ds-w-col-min`, — а не про класс: атрибут может остаться прежним, вкладки обязаны лечь в строку',
    viewport: frame,
    bundle: 'scripts/measure-tabs.tsx',
    html: ['1', '1.5'].map((sc) =>
    ...
    expect: (m) => {
      ...
      // ПРИКОЛОЧЕННЫЕ КОНЦЫ ДИАПАЗОНА. Без них законы выше держатся и у
      // раскладки, которая складывается ВСЕГДА (тогда боковой бар просто
      // исчез бы из системы), и у той, которая не складывается никогда, но
      // показывает вкладки покороче.
      if (frame === 360) {
        const h = m.hosts.find((x) => x.sc === 1.5)
        if (h && h.rows !== 1) bad.push(`×1.5: на телефоне лента осталась столбцом (${h.rows} ряда, бар ${h.bar ? h.bar.w : '?'} из ${h.host}) — порог не сработал`)
      }
      if (frame === 900) {
        for (const h of m.hosts) {
          if (h.rows !== h.tabs) bad.push(`×${h.sc}: на широком кадре лента уже не столбец (${h.rows} ряд(а)) — порог съел то, ради чего есть position`)
        }
      }
      return bad.length === 0 || bad.join('; ')
    },
