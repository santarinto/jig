 ...
 *
 * 360 ОСТАЁТСЯ (JIG-29, переезд пола 360 → 440, DS-380): найдено глазами
 * приёмкой DS-236, тем же приёмом, что «стрелка Tabs» и «полоса Pagination»
 * в CLAUDE.md — снятие замера теряет покрытие, а не экономит. На кадре 440
 * (хост 380) сценарий «ниже порога лента приходит к верхней» уже НЕ
 * воспроизводится ни на одной шкале (замерено `npm run measure`: rows 4 из 4
 * при шкале 1.5, было 1 при 360) — пол системы ушёл выше порога компонента,
 * и 440 в этом массиве остаётся общим ненаколоченным случаем без своей
 * пары `if (frame === …)`, как и был.
 */
...
// Хост 800 × 600, а не впритык: первая панель `flex: 0 1 auto` (DS-326)
  ...
  // vw, position:fixed и на @media у потребителя.
  {
    ...
    // НЕ ПЕРЕЕХАЛО НА 440 (JIG-29, сознательное отступление от буквы промта).
    // 320 здесь — не заявление о поддерживаемой ширине экрана, а ТЕХНИЧЕСКОЕ
    // условие самого замера: `min(22.5rem, 90vw)` обязан выбрать VW-ветку
    // ВНУТРИ кадра, чтобы её число ОТЛИЧАЛОСЬ от rem-ветки в общем документе
    // (360) — иначе пара не доказывает, что vw в кадре считается от кадра, а
    // не от окна раннера, она просто совпадает с ним по чистой случайности.
    // 90vw < 22.5rem только при ширине кадра < 400: на 440 (90vw = 396)
    // побеждает ТА ЖЕ rem-ветка, что и в документе (360 = 360), и утверждение
    // ниже (`frame === frameViewport*0.9`) стало бы КРАСНЫМ не от дефекта, а
    // от смены победившей ветки формулы — а формула, переписанная на `min()`,
    // сделала бы кадр и документ РАВНЫМИ и убила бы саму разницу, ради которой
    // случай заведён. Проверено `npm run measure` при временной подмене на
    // 440. Это МЕХАНИКА инструмента (`docs/writing-checks.md`), а не площадь
    // системного закона о ширине экрана — CLAUDE.md его не называет.
    html: `
  ...
  },
  {
    ...
    // 360/380/400 — те же исторические ширины, что у `TABS_ARROW_FRAMES`
    // (DS-236/284, найдено глазами; система обещает не ниже 440, но такую
    // находку не снимает, CLAUDE.md «System law: пол поддерживаемой ширины»).
    // Прежняя константа «кадр 360 минус поле хоста верстака» (= 300) СНЯТА
    // переездом пола (JIG-29): её замена, hostOf(440) = 380, дублирует уже
    // стоящую здесь ширину 380 и второй точки не добавляет.
    html: [360, 380, 400, 440, 520].flatMap((w) => ['1', '1.5'].map((sc) =>
    ...
    expect: (m) => {
      ...
      if (m.length !== 10) return `хостов ${m.length} вместо 10 — пять ширин на две шкалы`
      const bad = []
      let tight = 0, roomy = 0
      for (const r of m) {
