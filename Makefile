...
help:
	...
	@echo 'matrix     — матрица по всем случаям всех фикстур на 360: переполнение вбок, ширина поля, цель клика (в check-full); короткий вывод по умолчанию, полный — npm run matrix -- --all; одна строка — npm run matrix -- --row <ключ>'
