...
help:
	...
	@echo 'matrix     — матрица по всем случаям всех фикстур на 360: переполнение вбок, ширина поля, цель клика (в check-full); одна строка — npm run matrix -- --row <ключ>'
...
# фикстур на 360, каждая ячейка грузится один раз и отвечает всем строкам.
...
# соберёт CI (`npm run pack`), устанавливает первый, затем меняет specifier на
...
# (`npm run pack`, prepack/postpack режут devDependencies/scripts). Предмет
...
check: check-src
	@echo 'НЕ проверено: матрица по всем случаям на 360 — переполнение вбок, ширина поля ввода и цель клика (make matrix), установка тарболом и с --ignore-scripts, SSR без сборщика, команда обновления, поставка (сколько качает npm).'
...
# же тег пакует здесь `npm run pack`. Единственная цель, что трогает сеть —
