...
**Пол — это ОБЕЩАНИЕ, а не запрет мерить ниже.** Кадры 360, 380 и 400 остаются
там, где дефект однажды увидели глазами (`measure`: стрелка `Tabs`, полоса
`Pagination`) — снятие замера это работа и потеря покрытия, а не экономия.
Меняется одно: находка ПОД 440 больше не дефект системы, и задача по ней не
заводится. Находка НА 440 и выше — заводится, как прежде.
